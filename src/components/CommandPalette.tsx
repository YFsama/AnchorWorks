...
export function CommandPalette({
  ...
  const commands: Command[] = useMemo(() => [
    ...
    { id: 'cut.grommets',     label: t('Add Banner Grommets'), category: t('Arrange'), keywords: 'grommet eyelet banner hole finishing corner edge', icon: Wand2, run: () => { const paths = grommetsFromSelection(); if (paths.length) { const ed = useEditor.getState(); ed.addCutPaths(paths); ed.setCutPathsVisible(true); toast.success(`${paths.length} ${t('grommets added')}`); } else toast.warn(t('Select something first.')); } },
